    ...
    function structuredPatch(
      ...
      var _loop = function _loop2(i2) {
        ...
        if (current.added || current.removed) {
          ...
          (_curRange = /*istanbul ignore end*/ curRange).push.apply(
            /*istanbul ignore start*/
            _curRange,
            /*istanbul ignore start*/
            _toConsumableArray(
          ...
          if (oldRangeStart) {
            if (lines.length <= options.context * 2 && i2 < diff2.length - 2) {
              var _curRange2;
              (_curRange2 = /*istanbul ignore end*/ curRange).push.apply(
                /*istanbul ignore start*/
                _curRange2,
                /*istanbul ignore start*/
                _toConsumableArray(
              ...
              (_curRange3 = /*istanbul ignore end*/ curRange).push.apply(
                /*istanbul ignore start*/
                _curRange3,
                /*istanbul ignore start*/
                _toConsumableArray(
...
var require_common = __commonJS({
  "node_modules/glob/common.js"(exports) {
    ...
    function setopts(self, pattern, options) {
      ...
      if (!ownProp(options, "cwd")) self.cwd = cwd2;
      else {
    ...
    function isIgnored(self, path11) {
      ...
      return self.ignore.some(function (item) {
        return (
          item.matcher.match(path11) ||
          !!(item.gmatcher && item.gmatcher.match(path11))
        );
      });
    }
...
var require_rimraf = __commonJS({
  "node_modules/rimraf/rimraf.js"(exports, module) {
    ...
    var rmkidsSync = (p, options) => {
      ...
      options
        .readdirSync(p)
        .forEach((f) => rimrafSync(path10.join(p, f), options));
    ...
    var cache = {
      ...
      load: function (docId, cacheDir) {
        ...
        me._pathToFile = cacheDir
          ? path10.resolve(cacheDir, docId)
          : path10.resolve(__dirname, "../.cache/", docId);
  ...
  preprocess(value, utils) {
    if (
      ...
      if (suggestion) {
        utils.logger.warn(
          [
            `Unknown flag ${source_default.yellow(
              utils.descriptor.value(value),
            )},`,
            `did you mean ${source_default.blue(
...
async function getOptionsForFile(context, filepath) {
  ...
  context.logger.debug(
    `applied config-precedence (${
      context.argv.configPrecedence
...
function getNodeModuleDirectory(directory) {
  ...
  if (
    ...
    (fs3.existsSync(nodeModules) || !isWritable(path6.join(directory)))
  ) {
    return;
  }
  return nodeModules;
...
function findCacheDirectory(options = {}) {
  ...
  if (options.files) {
    directory = (0, import_common_path_prefix.default)(
      options.files.map((file) => path6.resolve(directory, file)),
    );
  }
  ...
  return useDirectory(
    path6.join(directory, "node_modules", ".cache", options.name),
    options,
  );
}
...
function getHashOfOptions(options) {
  ...
  const hash = createHash(
    ...
    import_fast_json_stable_stringify.default)(options)}`,
  );
...
function diff(a, b) {
  return (0, import_diff.createTwoFilesPatch)("", "", a, b, "", "", {
    context: 2,
  });
}
...
function handleError(context, filename, error, printedFilename) {
  ...
  const isValidationError = /^Invalid \S+ value\./.test(
...
${error.message}`,
        ...
        const message = `${source_default.grey(fileNameToDisplay)} ${
          Date.now() - start
        }ms`;
        if (isCacheExists) {
...
async function logFileInfoOrDie(context) {
  ...
  printToScreen(
    await format3((0, import_fast_json_stable_stringify2.default)(fileInfo), {
      parser: "json",
    }),
  );
}
